...
// Function to find a balanced block starting at a specific substring
...
const pricingBlock = extractBlock("{activeTab === 'pricing' && (", '{', '}')
const faqBlock = extractBlock("{activeTab === 'faq' && (", '{', '}')
const fleetBlock = extractBlock("{activeTab === 'fleet' && (", '{', '}')
const servicesBlock = extractBlock("{activeTab === 'services' && (", '{', '}')
const whyBlock = extractBlock("{activeTab === 'why' && (", '{', '}')

// The blocks include `{activeTab === ... && (` and `)}`. We need to extract the inner HTML.
function getInnerHtml(block) {
  if (!block) return ''
  // Remove `{activeTab === 'xyz' && (` from start and `)}` from end
  const firstParen = block.indexOf('(')
  // The block ends with `)}`
  let inner = block.substring(firstParen + 1, block.lastIndexOf(')')).trim()
  return inner
}

const pricingHTML = getInnerHtml(pricingBlock)
const faqHTML = getInnerHtml(faqBlock)
const fleetHTML = getInnerHtml(fleetBlock)
const servicesHTML = getInnerHtml(servicesBlock)
const whyHTML = getInnerHtml(whyBlock)

if (!pricingHTML || !faqHTML || !fleetHTML || !servicesHTML || !whyHTML) {
  console.log("Failed to extract one or more blocks!")
  process.exit(1)
}

// Remove Pricing and FAQ blocks from the document
content = content.replace(pricingBlock, '')
content = content.replace(faqBlock, '')

// Remove Pricing and FAQ from the Tabs array
content = content.replace(/\{\s*id: 'pricing',[\s\S]*?\},/, '')
content = content.replace(/\{\s*id: 'faq',[\s\S]*?\}\s*\]/, ']')
...
fs.writeFileSync('app/hotel/[slug]/page.tsx.safe', content)
console.log("Refactored to page.tsx.safe")
