...
export default function Testimonials({ reviews }: { reviews?: Testimonial[] }) {
  ...
  return (
    <section className="w-full py-24" style={{ borderTop: '1px solid var(--surface)', background: 'var(--bg-alt-2)' }}>
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-500">
          {visibleItems.map((t, idx) => (
            <div
              key={t.name + idx}
              className={`rounded-2xl p-8 flex flex-col justify-between transition-all duration-500 hover:-translate-y-1 hover:border-[var(--gold)] hover:shadow-lg hover:shadow-[#B8960C08] ${
